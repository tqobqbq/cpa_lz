...
//     Explicit proxy override used for this one request. This has higher priority than the selected
//     credential proxy and the global proxy-url setting.
...
func (h *Handler) APICall(c *gin.Context) {
	...
	proxyOverride := firstNonEmptyString(body.ProxyURLSnake, body.ProxyURLCamel, body.ProxyURLPascal)
	...
	httpClient.Transport = h.apiCallTransport(auth, proxyOverride)
...
func (h *Handler) apiCallTransport(auth *coreauth.Auth, proxyOverrides ...string) http.RoundTripper {
	var proxyCandidates []string
	for _, proxyOverride := range proxyOverrides {
