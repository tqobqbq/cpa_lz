...
func (h *Handler) buildAuthFileEntry(auth *coreauth.Auth) gin.H {
	...
	entry := gin.H{
		...
		"label":          auth.Label,
		"status":         auth.Status,
		"status_message": authRuntimeStatusMessage(auth),
	...
	if lastError := buildRuntimeErrorEntry(auth.LastError); lastError != nil {
		entry["last_error"] = lastError
	}
	if quota := buildQuotaStateEntry(auth.Quota); quota != nil {
		entry["quota"] = quota
	}
	if modelStates := buildModelStatesEntry(auth.ModelStates); modelStates != nil {
	...
	if auth.Metadata != nil {
		if rawMode, ok := auth.Metadata["backoff_mode"].(string); ok {
			if trimmed := strings.TrimSpace(rawMode); trimmed != "" {
				entry["backoff_mode"] = trimmed
			}
		} else if rawMode, ok := auth.Metadata["backoff-mode"].(string); ok {
			if trimmed := strings.TrimSpace(rawMode); trimmed != "" {
				entry["backoff_mode"] = trimmed
			}
		}
		if rawRetry, ok := auth.Metadata["request_retry"]; ok {
			switch v := rawRetry.(type) {
			case float64:
				entry["request_retry"] = int(v)
			case int:
				entry["request_retry"] = v
			case string:
				if parsed, err := strconv.Atoi(strings.TrimSpace(v)); err == nil {
					entry["request_retry"] = parsed
				}
			}
		} else if rawRetry, ok := auth.Metadata["request-retry"]; ok {
			switch v := rawRetry.(type) {
			case float64:
				entry["request_retry"] = int(v)
			case int:
...
func authRuntimeStatusMessage(auth *coreauth.Auth) string {
	if auth == nil {
		return ""
	}
	...
	if auth.LastError != nil {
		if msg := strings.TrimSpace(auth.LastError.Message); msg != "" {
			return msg
		}
		if msg := strings.TrimSpace(auth.LastError.Error()); msg != "" {
			return msg
		}
	}
	if msg := modelStatesStatusMessage(auth.ModelStates); msg != "" {
		return msg
	}
	if msg := quotaStateStatusMessage(auth.Quota); msg != "" {
		return msg
	}
	if auth.Unavailable {
		return retryAfterStatusMessage("temporarily unavailable", auth.NextRetryAfter)
	}
	return ""
}

func buildRuntimeErrorEntry(err *coreauth.Error) gin.H {
	if err == nil {
		return nil
	}
	entry := gin.H{}
...
func buildQuotaStateEntry(state coreauth.QuotaState) gin.H {
	if quotaStateIsZeroForManagement(state) {
		return nil
	}
	entry := gin.H{}
	if state.Exceeded {
	...
	if !state.NextRecoverAt.IsZero() {
		entry["next_recover_at"] = state.NextRecoverAt
	}
	if state.BackoffLevel != 0 {
		entry["backoff_level"] = state.BackoffLevel
	}
	return entry
}

func buildModelStatesEntry(states map[string]*coreauth.ModelState) gin.H {
	if len(states) == 0 {
		return nil
	}
	...
	for _, model := range keys {
		if entry := buildModelStateEntry(states[model]); entry != nil {
			out[model] = entry
		}
	}
	if len(out) == 0 {
...
func buildModelStateEntry(state *coreauth.ModelState) gin.H {
	if state == nil {
		return nil
	}
	entry := gin.H{}
	...
	if lastError := buildRuntimeErrorEntry(state.LastError); lastError != nil {
		entry["last_error"] = lastError
	}
	if quota := buildQuotaStateEntry(state.Quota); quota != nil {
		entry["quota"] = quota
	}
...
func modelStatesStatusMessage(states map[string]*coreauth.ModelState) string {
	if len(states) == 0 {
		return ""
	}
	keys := make([]string, 0, len(states))
	...
	for _, model := range keys {
		if msg := modelStateStatusMessage(model, states[model]); msg != "" {
...
func modelStateStatusMessage(model string, state *coreauth.ModelState) string {
	if state == nil {
		return ""
	}
	message := strings.TrimSpace(state.StatusMessage)
	if message == "" && state.LastError != nil {
		message = strings.TrimSpace(state.LastError.Message)
	}
	if message == "" {
		message = quotaStateStatusMessage(state.Quota)
	}
	if message == "" && state.Unavailable {
		message = retryAfterStatusMessage("temporarily unavailable", state.NextRetryAfter)
	}
	if message == "" && state.Status != "" && state.Status != coreauth.StatusActive {
...
func quotaStateStatusMessage(state coreauth.QuotaState) string {
	if !quotaStateHasVisibleStatus(state) {
		return ""
	}
	...
	return retryAfterStatusMessage(reason, state.NextRecoverAt)
}

func retryAfterStatusMessage(prefix string, retryAfter time.Time) string {
	prefix = strings.TrimSpace(prefix)
	if prefix == "" {
		prefix = "temporarily unavailable"
	}
	if retryAfter.IsZero() {
		return prefix
	}
	return fmt.Sprintf("%s until %s", prefix, retryAfter.UTC().Format(time.RFC3339))
}

func quotaStateIsZeroForManagement(state coreauth.QuotaState) bool {
	return !state.Exceeded && strings.TrimSpace(state.Reason) == "" && state.NextRecoverAt.IsZero() && state.BackoffLevel == 0
}

func quotaStateHasVisibleStatus(state coreauth.QuotaState) bool {
	return state.Exceeded || strings.TrimSpace(state.Reason) != "" || !state.NextRecoverAt.IsZero()
}
...
func (h *Handler) PatchAuthFileFields(c *gin.Context) {
	...
	if req.Priority != nil || req.Note != nil || req.BackoffMode != nil || req.RequestRetry != nil {
		if targetAuth.Metadata == nil {
			targetAuth.Metadata = make(map[string]any)
		}
		if targetAuth.Attributes == nil {
		...
		backoffMode := ""
		if req.BackoffMode != nil {
			backoffMode = normalizeBackoffMode(*req.BackoffMode)
			if backoffMode == "default" {
				delete(targetAuth.Metadata, "backoff_mode")
			} else {
				targetAuth.Metadata["backoff_mode"] = backoffMode
			}
			if backoffMode != "custom" {
				delete(targetAuth.Metadata, "request_retry")
			}
		} else if existingMode, ok := targetAuth.Metadata["backoff_mode"].(string); ok {
			backoffMode = normalizeBackoffMode(existingMode)
		}
		if req.RequestRetry != nil {
			if *req.RequestRetry < 0 {
				c.JSON(http.StatusBadRequest, gin.H{"error": "request_retry must be non-negative"})
				return
			}
			if backoffMode == "custom" {
				targetAuth.Metadata["request_retry"] = *req.RequestRetry
			}
		}
		if req.Note != nil {
...
func (h *Handler) RequestGeminiCLIToken(c *gin.Context) {
	...
	clientID, clientSecret, errCredentials := geminiAuth.OAuthClientCredentials()
	if errCredentials != nil {
		log.WithError(errCredentials).Error("missing gemini oauth client credentials")
		c.JSON(http.StatusInternalServerError, gin.H{"error": errCredentials.Error()})
		return
	}
