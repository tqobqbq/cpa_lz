...
func (h *Handler) GetConfig(c *gin.Context) {
	...
	// Serialize base config to a generic map so we can selectively
	// replace provider key arrays with auth-index-enriched versions.
	...
	// Overwrite provider key arrays with wrappers that include auth-index.
	response["gemini-api-key"] = h.geminiKeysWithAuthIndex()
	response["claude-api-key"] = h.claudeKeysWithAuthIndex()
	response["codex-api-key"] = h.codexKeysWithAuthIndex()
	response["vertex-api-key"] = h.vertexCompatKeysWithAuthIndex()
	response["openai-compatibility"] = h.openAICompatibilityWithAuthIndex()
...
func (h *Handler) GetRoutingRules(c *gin.Context) {
	c.JSON(200, gin.H{"rules": h.cfg.Routing.Rules})
}

func (h *Handler) PutRoutingRules(c *gin.Context) {
	var rules []config.RoutingRule
	if errBindJSON := c.ShouldBindJSON(&rules); errBindJSON != nil {
		var body struct {
			Rules []config.RoutingRule `json:"rules"`
		}
		if errWrapped := c.ShouldBindJSON(&body); errWrapped != nil {
			c.JSON(http.StatusBadRequest, gin.H{"error": "invalid body"})
			return
		}
		rules = body.Rules
	...
	normalized := make([]config.RoutingRule, 0, len(rules))
	for _, rule := range rules {
		provider := strings.ToLower(strings.TrimSpace(rule.Provider))
		userAgentMatch := strings.ToLower(strings.TrimSpace(rule.UserAgent.Match))
		userAgentValue := strings.TrimSpace(rule.UserAgent.Value)
		inputOperator := strings.ToLower(strings.TrimSpace(rule.InputChars.Operator))
		inputValue := rule.InputChars.Value

		if provider == "" {
			c.JSON(http.StatusBadRequest, gin.H{"error": "routing rule provider is required"})
			return
		}
		normalizedRule := config.RoutingRule{Provider: provider}

		if userAgentMatch != "" || userAgentValue != "" {
			switch userAgentMatch {
			case "contains":
			case "equals", "eq":
				userAgentMatch = "equals"
			default:
				c.JSON(http.StatusBadRequest, gin.H{"error": "routing rule user-agent match must be equals or contains"})
				return
			}
			if userAgentValue == "" {
				c.JSON(http.StatusBadRequest, gin.H{"error": "routing rule user-agent value is required"})
				return
			}
			normalizedRule.UserAgent = config.RoutingUserAgentRule{
				Match: userAgentMatch,
				Value: userAgentValue,
			}
		}

		if inputOperator != "" || rule.InputChars.Value != 0 {
			...
			case "lt", "less-than", "less_than":
				inputOperator = "lt"
			default:
				c.JSON(http.StatusBadRequest, gin.H{"error": "routing rule input-chars operator must be gt or lt"})
				return
			}
			if inputValue < 0 {
				c.JSON(http.StatusBadRequest, gin.H{"error": "routing rule input-chars value must be non-negative"})
				return
			}
			normalizedRule.InputChars = config.RoutingInputCharsRule{
	...
	h.cfg.Routing.Rules = normalized
	h.persist(c)
}
