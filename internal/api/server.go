...
func (s *Server) registerManagementRoutes() {
	...
	mgmt.Use(s.managementAvailabilityMiddleware(), s.mgmt.Middleware())
	{
		...
		mgmt.GET("/force-retryable-errors", s.mgmt.GetForceRetryableErrors)
		mgmt.PUT("/force-retryable-errors", s.mgmt.PutForceRetryableErrors)
		mgmt.PATCH("/force-retryable-errors", s.mgmt.PutForceRetryableErrors)
		mgmt.GET("/codex-remove-empty-input-name", s.mgmt.GetCodexRemoveEmptyInputName)
		mgmt.PUT("/codex-remove-empty-input-name", s.mgmt.PutCodexRemoveEmptyInputName)
		mgmt.PATCH("/codex-remove-empty-input-name", s.mgmt.PutCodexRemoveEmptyInputName)

		mgmt.GET("/routing/strategy", s.mgmt.GetRoutingStrategy)
		mgmt.PUT("/routing/strategy", s.mgmt.PutRoutingStrategy)
		mgmt.PATCH("/routing/strategy", s.mgmt.PutRoutingStrategy)
		mgmt.GET("/routing/rules", s.mgmt.GetRoutingRules)
		mgmt.PUT("/routing/rules", s.mgmt.PutRoutingRules)
		mgmt.PATCH("/routing/rules", s.mgmt.PutRoutingRules)
		...
		mgmt.GET("/auth-files-group", s.mgmt.GetAuthFilesGroup)
		mgmt.PUT("/auth-files-group", s.mgmt.PutAuthFilesGroup)
		mgmt.PATCH("/auth-files-group", s.mgmt.PatchAuthFilesGroup)
...
func (s *Server) serveManagementControlPanel(c *gin.Context) {
	...
	// An explicit MANAGEMENT_STATIC_PATH override takes precedence over the embedded panel.
	if override := strings.TrimSpace(os.Getenv("MANAGEMENT_STATIC_PATH")); override != "" {
		if data, errRead := os.ReadFile(managementasset.FilePath("")); errRead == nil && len(data) > 0 {
			c.Header("Cache-Control", "no-store")
			c.Data(http.StatusOK, "text/html; charset=utf-8", data)
			return
		}
	}
	data, errEmbedded := managementasset.EmbeddedHTML()
	if errEmbedded != nil {
		log.WithError(errEmbedded).Error("embedded management panel unavailable")
		c.AbortWithStatusJSON(http.StatusServiceUnavailable, gin.H{"error": "management panel unavailable"})
		return
	}
