...
func TestManagementUsageRequiresManagementAuthAndPopsArray(t *testing.T) {
	...
	// The aggregated usage statistics endpoint is retained in this fork; it must
	// respond for authenticated management requests.
	...
	// And it must reject unauthenticated requests like every other management route.
	statsNoAuthReq := httptest.NewRequest(http.MethodGet, "/v0/management/usage?count=2", nil)
	statsNoAuthRR := httptest.NewRecorder()
	server.engine.ServeHTTP(statsNoAuthRR, statsNoAuthReq)
	if statsNoAuthRR.Code != http.StatusUnauthorized {
		t.Fatalf("unauthenticated usage statistics status = %d, want %d body=%s", statsNoAuthRR.Code, http.StatusUnauthorized, statsNoAuthRR.Body.String())
	}
