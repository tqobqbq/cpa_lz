...
attemptLoop:
	for attempt := 0; attempt < attempts; attempt++ {
		...
		for idx, baseURL := range baseURLs {
			...
			reporter.SetUpstreamModelFromPayload(bodyBytes)
			...
			if _, errValidate := helps.ValidateDownstreamNonStreamPayloadWithOutputFilter(responseFormat, e.Identifier(), converted, helps.OutputFilterFromConfig(e.cfg)); errValidate != nil {
		...
		for idx, baseURL := range baseURLs {
			...
			go func(resp *http.Response) {
				...
				if errScan := scanner.Err(); errScan != nil {
					...
					errEmpty := helps.NewResponseFormatError(http.StatusOK, "upstream_empty_response", "antigravity upstream returned an empty response")
			...
			reporter.SetUpstreamModelFromPayload(resp.Payload)
	...
	for attempt := 0; attempt < attempts; attempt++ {
		...
		for idx, baseURL := range baseURLs {
			...
			go func(resp *http.Response) {
				...
				if errScan := scanner.Err(); errScan == nil && payloadCount == 0 {
					errEmpty := helps.NewResponseFormatError(http.StatusOK, "upstream_empty_response", "antigravity upstream returned an empty response")
					helps.RecordAPIResponseError(ctx, e.cfg, errEmpty)
					reporter.PublishFailure(ctx, errEmpty)
					select {
