...
func (e *ClaudeExecutor) Execute(ctx context.Context, auth *cliproxyauth.Auth, req cliproxyexecutor.Request, opts cliproxyexecutor.Options) (resp cliproxyexecutor.Response, err error) {
	...
	if stream {
		...
		for _, line := range lines {
			if errValidate := helps.ValidateDownstreamStreamChunkWithOutputFilterForProvider(sdktranslator.FormatClaude, e.Identifier(), line, helps.OutputFilterFromConfig(e.cfg)); errValidate != nil {
	...
	if _, errValidate := helps.ValidateDownstreamNonStreamPayloadWithOutputFilter(responseFormat, e.Identifier(), out, helps.OutputFilterFromConfig(e.cfg)); errValidate != nil {
		reporter.PublishFailure(ctx, errValidate)
		return resp, errValidate
	}
...
func (e *ClaudeExecutor) ExecuteStream(ctx context.Context, auth *cliproxyauth.Auth, req cliproxyexecutor.Request, opts cliproxyexecutor.Options) (_ *cliproxyexecutor.StreamResult, err error) {
	...
	go func() {
		...
		if responseFormat == to {
			...
			var semanticState claudeStreamSemanticState
			var pending [][]byte
			emitOrBuffer := func(chunk []byte) bool {
				if semanticState.HasOutput() {
					for _, buffered := range pending {
						select {
						case out <- cliproxyexecutor.StreamChunk{Payload: buffered}:
						case <-ctx.Done():
							return false
						}
					}
			...
			for scanner.Scan() {
				...
				if errValidate := helps.ValidateDownstreamStreamChunkWithOutputFilterForProvider(sdktranslator.FormatClaude, e.Identifier(), line, helps.OutputFilterFromConfig(e.cfg)); errValidate != nil {
					reporter.PublishFailure(ctx, errValidate)
					select {
					case out <- cliproxyexecutor.StreamChunk{Err: errValidate}:
					case <-ctx.Done():
					}
					return
				}
			...
			if errEmpty := semanticState.EmptyResponseError(e.Identifier()); errEmpty != nil {
				...
				select {
				case out <- cliproxyexecutor.StreamChunk{Err: errEmpty}:
				case <-ctx.Done():
				}
				return
			}
		...
		for scanner.Scan() {
			...
			for i := range chunks {
				if errValidate := helps.ValidateDownstreamStreamChunkWithOutputFilterForProvider(responseFormat, e.Identifier(), chunks[i], helps.OutputFilterFromConfig(e.cfg)); errValidate != nil {
					reporter.PublishFailure(ctx, errValidate)
					select {
					case out <- cliproxyexecutor.StreamChunk{Err: errValidate}:
					case <-ctx.Done():
...
func applyClaudeHeaders(r *http.Request, auth *cliproxyauth.Auth, apiKey string, stream bool, extraBetas []string, cfg *config.Config, incomingHeaders http.Header) error {
	...
	useAPIKey := shouldUseClaudeAPIKeyHeader(r, auth)
...
// shouldUseClaudeAPIKeyHeader decides whether the upstream request should carry the
// credential via the x-api-key header instead of an Authorization bearer token.
// The auth entry may force a mode via the "auth_mode" attribute; otherwise the
// official Anthropic endpoint defaults to x-api-key for API keys while custom base
// URLs keep the bearer form.
func shouldUseClaudeAPIKeyHeader(r *http.Request, auth *cliproxyauth.Auth) bool {
	if auth == nil || auth.Attributes == nil || strings.TrimSpace(auth.Attributes["api_key"]) == "" {
		return false
	}
	switch normalizeClaudeAuthMode(auth.Attributes["auth_mode"]) {
	case "api-key":
		return true
	case "bearer":
		return false
	}
	return r != nil && r.URL != nil && strings.EqualFold(r.URL.Scheme, "https") && strings.EqualFold(r.URL.Host, "api.anthropic.com")
}

// normalizeClaudeAuthMode maps configured auth-mode aliases onto canonical values.
...
// claudeStreamSemanticState tracks whether a Claude SSE stream carried any semantic
// output. Streams that report usage but never emit content are treated as empty
// responses so the scheduler can retry another credential instead of returning a
// blank completion to the client.
type claudeStreamSemanticState struct {
	usageSeen    bool
	outputTokens int64
	hasOutput    bool
	stopSeen     bool
...
func (s *claudeStreamSemanticState) Observe(line []byte) {
	if s == nil {
		return
	}
	s.linesScanned++
	payload := claudeStreamDataPayload(line)
	if len(payload) == 0 || !gjson.ValidBytes(payload) {
		return
	}
	root := gjson.ParseBytes(payload)
	if usage := claudeStreamUsageNode(root); usage.Exists() {
		s.usageSeen = true
...
func (s claudeStreamSemanticState) EmptyResponseError(provider string) error {
	...
	// 1. usageSeen but no output (upstream sent usage event with zero content)
	// 2. No lines scanned at all (upstream returned 200 with completely empty body)
	if !s.usageSeen && s.linesScanned > 0 {
		return nil
	}
...
func claudeStreamUsageNode(root gjson.Result) gjson.Result {
	if usage := root.Get("usage"); usage.Exists() {
		return usage
	}
	return root.Get("message.usage")
}

func claudeStreamHasSemanticOutput(root gjson.Result) bool {
	if claudeContentBlockHasSemanticOutput(root.Get("content_block")) {
		return true
	}
	if claudeDeltaHasSemanticOutput(root.Get("delta")) {
		return true
	}
	...
	for _, block := range content.Array() {
		if claudeContentBlockHasSemanticOutput(block) {
			return true
		}
	}
	return false
...
func claudeContentBlockHasSemanticOutput(block gjson.Result) bool {
	if !block.Exists() {
		return false
	}
...
func claudeDeltaHasSemanticOutput(delta gjson.Result) bool {
	if !delta.Exists() {
		return false
	}
	deltaType := strings.TrimSpace(delta.Get("type").String())
	switch deltaType {
	case "text_delta":
		return strings.TrimSpace(delta.Get("text").String()) != ""
	case "input_json_delta":
		return strings.TrimSpace(delta.Get("partial_json").String()) != ""
	case "thinking_delta":
		return strings.TrimSpace(delta.Get("thinking").String()) != ""
	default:
		return false
	}
}
...
// shouldIgnoreClaudeStreamEOF tolerates an unexpected EOF that arrives after the
// stream already delivered its terminal message_stop event; everything the client
// needs has been forwarded at that point.
