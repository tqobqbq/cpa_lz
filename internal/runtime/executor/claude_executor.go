...
func (e *ClaudeExecutor) Execute(ctx context.Context, auth *cliproxyauth.Auth, req cliproxyexecutor.Request, opts cliproxyexecutor.Options) (resp cliproxyexecutor.Response, err error) {
	...
	if stream {
		...
		reporter.SetDetail(helps.ParseClaudeUsage(data))
	}
...
func (e *ClaudeExecutor) ExecuteStream(ctx context.Context, auth *cliproxyauth.Auth, req cliproxyexecutor.Request, opts cliproxyexecutor.Options) (_ *cliproxyexecutor.StreamResult, err error) {
	...
	go func() {
		...
		if from == to {
			...
			var pending [][]byte
			emitOrBuffer := func(chunk []byte) {
				if semanticState.HasOutput() {
					for _, buffered := range pending {
						out <- cliproxyexecutor.StreamChunk{Payload: buffered}
					}
					pending = nil
			...
			for scanner.Scan() {
				...
				copy(cloned, line)
				cloned[len(line)] = '\n'
				if errValidate := helps.ValidateDownstreamStreamChunkWithOutputFilterForProvider(from, e.Identifier(), cloned, helps.OutputFilterFromConfig(e.cfg)); errValidate != nil {
					reporter.PublishFailureWithError(ctx, errValidate)
					out <- cliproxyexecutor.StreamChunk{Err: errValidate}
					return
				}
...
func (e *ClaudeExecutor) CountTokens(ctx context.Context, auth *cliproxyauth.Auth, req cliproxyexecutor.Request, opts cliproxyexecutor.Options) (cliproxyexecutor.Response, error) {
	...
	url := strings.TrimRight(baseURL, "/") + "/v1/messages/count_tokens?beta=true"
...
func (e *ClaudeExecutor) Refresh(ctx context.Context, auth *cliproxyauth.Auth) (*cliproxyauth.Auth, error) {
	...
	svc := claudeauth.NewClaudeAuthWithProxyURL(nil, auth.ProxyURL)
...
func applyClaudeHeaders(r *http.Request, auth *cliproxyauth.Auth, apiKey string, stream bool, extraBetas []string, cfg *config.Config) {
	...
	if shouldUseClaudeAPIKeyHeader(r, auth) {
