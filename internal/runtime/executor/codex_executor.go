...
func (e *CodexExecutor) Execute(ctx context.Context, auth *cliproxyauth.Auth, req cliproxyexecutor.Request, opts cliproxyexecutor.Options) (resp cliproxyexecutor.Response, err error) {
	...
	url := buildCodexResponsesURL(baseURL, codexUseV1(auth), "/responses")
	...
	for _, line := range lines {
		...
		if streamErr, ok := codexStreamErrorFromSSE("", eventData); ok {
			err = streamErr
			return resp, err
		}
	...
	if len(bytes.TrimSpace(upstreamData)) == 0 {
...
func (e *CodexExecutor) executeCompact(ctx context.Context, auth *cliproxyauth.Auth, req cliproxyexecutor.Request, opts cliproxyexecutor.Options) (resp cliproxyexecutor.Response, err error) {
	...
	url := buildCodexResponsesURL(baseURL, codexUseV1(auth), "/responses/compact")
	var identityState codexIdentityConfuseState
	httpReq, upstreamBody, identityState, err := e.cacheHelper(ctx, from, url, auth, req, originalPayloadSource, body)
	if err != nil {
		return resp, err
	...
	upstreamData := applyCodexIdentityConfuseResponsePayload(data, identityState)
	helps.AppendAPIResponseChunk(ctx, e.cfg, upstreamData)
	reporter.SetUpstreamModelFromPayload(upstreamData)
	...
	go func() {
		...
		for scanner.Scan() {
			...
			if bytes.HasPrefix(line, dataTag) {
				...
				currentEvent = ""
				var typeRepaired bool
				data, _, typeRepaired = codexNormalizeEventType(eventName, data)
				...
				if errEmptyEvent := codexEmptyDataEventError(eventName, data); errEmptyEvent != nil {
					...
					reporter.PublishFailure(ctx, errEmptyEvent)
					select {
					case out <- cliproxyexecutor.StreamChunk{Err: errEmptyEvent}:
					case <-ctx.Done():
					}
					return
				}
		...
		if linesScanned == 0 {
			...
			helps.RecordAPIResponseError(ctx, e.cfg, errEmpty)
		...
		if compatibilityRepairErr != nil {
			// The stream already completed for the client; surface the repair in
			// request logs without emitting a second usage record.
			helps.RecordAPIResponseError(ctx, e.cfg, compatibilityRepairErr)
			helps.LogWithRequestID(ctx).Warnf("codex executor: %v", compatibilityRepairErr)
		}
	}()
...
func publishCodexImageToolUsage(ctx context.Context, reporter *helps.UsageReporter, body []byte, completedData []byte) {
	reporter.SetUpstreamModelFromPayload(completedData)
...
// codexUseV1 reports whether the auth entry expects the OpenAI-style /v1 path
// segment appended to its base URL. Config-derived API-key entries default to
// true via the synthesizer (`use-v1` option); auths without the attribute
// (e.g. OAuth credentials against the official backend) never get /v1.
...
// normalizeCodexRequestBody applies the shared instruction normalization and,
// when enabled, strips empty `name` fields from input items. Some Codex-compatible
// upstreams reject input items whose name is present but blank; function_call
// items cannot survive without a name, so they are dropped entirely.
...
// codexResponseMetadataState carries the response identity across SSE events so
// missing metadata can be repaired consistently within one stream.
...
// codexNormalizeEventType stamps the SSE event name into the data payload's type
// field when the upstream omitted it. Only response.* event names are stamped.
...
// codexStreamErrorFromSSE converts SSE error frames (event: error lines, error /
// response.failed data events, embedded error objects) into a statusErr so the
// scheduler can rotate credentials instead of forwarding the failure downstream.
// It is broader than codexTerminalStreamErr, which only intercepts a curated set
// of terminal error classes; run it after that check as a fallback.
...
// patchCodexResponseMetadata fills in required Responses SSE metadata (response.id,
// object, created_at, model, status, ...) that some Codex-compatible upstreams omit,
// keeping downstream OpenAI Responses clients from rejecting the stream.
