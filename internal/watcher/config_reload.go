...
func (w *Watcher) reloadConfig() bool {
	...
	resolvedAuthDir, errResolveAuthDir := w.resolveAuthDir(newConfig)
	...
	if authDirChanged {
		if errSwitch := w.switchAuthDir(newConfig.AuthDir); errSwitch != nil {
			log.Errorf("failed to switch auth directory to %s: %v", newConfig.AuthDir, errSwitch)
			return false
		}
	}
