...
func BuildConfigChangeDetails(oldCfg, newCfg *config.Config) []string {
	...
	if !reflect.DeepEqual(oldCfg.ErrorControl, newCfg.ErrorControl) {
		changes = append(changes, "error-control changed")
	}
	...
	if len(oldCfg.ClaudeKey) != len(newCfg.ClaudeKey) {
		...
		for i := range oldCfg.ClaudeKey {
			...
			if strings.TrimSpace(o.AuthMode) != strings.TrimSpace(n.AuthMode) {
				changes = append(changes, fmt.Sprintf("claude[%d].auth-mode: %s -> %s", i, strings.TrimSpace(o.AuthMode), strings.TrimSpace(n.AuthMode)))
			}
			if strings.TrimSpace(o.ProxyURL) != strings.TrimSpace(n.ProxyURL) {
