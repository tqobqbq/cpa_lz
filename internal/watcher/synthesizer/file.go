...
func synthesizeFileAuths(ctx *SynthesisContext, fullPath string, data []byte) []*coreauth.Auth {
	...
	if strings.TrimSpace(proxyURL) == "" && cfg != nil {
	...
	disabled, _ := metadata["disabled"].(bool)
	if cfg != nil && !config.EffectiveBool(cfg.AuthFilesGroup.Enabled, true) {
		disabled = true
	}
	...
	// Fall back to the group-level auth files priority when the file does not set one.
	if _, ok := a.Attributes["priority"]; !ok && cfg != nil {
		if groupPriority := config.EffectivePriority(cfg.AuthFilesGroup.Priority); groupPriority != 0 {
			a.Attributes["priority"] = strconv.Itoa(groupPriority)
		}
	}
