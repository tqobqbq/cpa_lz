...
func (w *Watcher) SetConfig(cfg *config.Config) {
	if cfg != nil {
		if resolvedAuthDir, errResolveAuthDir := w.resolveAuthDir(cfg); errResolveAuthDir != nil {
			log.Errorf("failed to resolve auth directory from config: %v", errResolveAuthDir)
		} else {
			cfg.AuthDir = resolvedAuthDir
			if errSwitch := w.switchAuthDir(resolvedAuthDir); errSwitch != nil {
				log.Errorf("failed to switch auth directory to %s: %v", resolvedAuthDir, errSwitch)
			}
		}
	}
	w.clientsMutex.Lock()
...
func (w *Watcher) resolveAuthDir(cfg *config.Config) (string, error) {
	...
	if w.mirroredAuthDir != "" {
		return filepath.Clean(strings.TrimSpace(w.mirroredAuthDir)), nil
	}
...
func (w *Watcher) switchAuthDir(nextAuthDir string) error {
	...
	currentAuthDir := filepath.Clean(strings.TrimSpace(w.authDir))
	authDirWatched := w.authDirWatched
	w.clientsMutex.RUnlock()

	if currentAuthDir == nextAuthDir {
		return nil
	}

	if authDirWatched && w.watcher != nil {
		if errAddAuthDir := w.watcher.Add(nextAuthDir); errAddAuthDir != nil {
			return fmt.Errorf("watch auth directory %s: %w", nextAuthDir, errAddAuthDir)
		}
		if currentAuthDir != "" {
			if errRemoveAuthDir := w.watcher.Remove(currentAuthDir); errRemoveAuthDir != nil {
				log.WithError(errRemoveAuthDir).Warnf("failed to remove previous auth directory watch: %s", currentAuthDir)
			}
		}
		log.Debugf("switched watched auth directory from %s to %s", currentAuthDir, nextAuthDir)
	}
