...
func requestExecutionMetadata(ctx context.Context, rawJSON []byte) map[string]any {
	...
	requestPath := ""
	userAgent := ""
	inputChars := int64(utf8.RuneCount(rawJSON))
	if ctx != nil {
		if ginCtx, ok := ctx.Value("gin").(*gin.Context); ok && ginCtx != nil && ginCtx.Request != nil {
			key = strings.TrimSpace(ginCtx.GetHeader("Idempotency-Key"))
			userAgent = strings.TrimSpace(ginCtx.Request.UserAgent())
			ginCtx.Set("request_input_chars", inputChars)
	...
	if userAgent != "" {
		meta[coreexecutor.RequestUserAgentMetadataKey] = userAgent
	}
	meta[coreexecutor.RequestInputCharsMetadataKey] = inputChars
...
func (h *BaseAPIHandler) applyModelRouter(ctx context.Context, handlerType, modelName string, rawJSON []byte, stream bool, execOptions modelExecutionOptions) modelRouteDecision {
	...
	meta := requestExecutionMetadata(ctx, rawJSON)
