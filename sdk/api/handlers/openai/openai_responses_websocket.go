...
func (h *OpenAIResponsesAPIHandler) ResponsesWebsocket(c *gin.Context) {
	...
	for {
		...
		if errMsg != nil {
			...
			errorPayload, errWrite := writeResponsesWebsocketError(writer, wsTimelineLog, h.Cfg, errMsg)
			log.Infof(
	...
	for {
		...
		case errMsg, ok := <-errs:
			...
			if errMsg != nil {
				...
				errorPayload, errWrite := writeResponsesWebsocketError(writer, wsTimelineLog, cfg, errMsg)
				log.Infof(
					"responses websocket: downstream_out id=%s type=%d event=%s payload=%s",
					sessionID,
					websocket.TextMessage,
...
func writeResponsesWebsocketError(writer *responsesWebsocketWriter, wsTimelineLog websocketTimelineAppender, cfg *sdkconfig.SDKConfig, errMsg *interfaces.ErrorMessage) ([]byte, error) {
