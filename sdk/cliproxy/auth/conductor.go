...
func (m *Manager) executeStreamWithModelPool(ctx context.Context, executor ProviderExecutor, auth *Auth, provider string, req cliproxyexecutor.Request, opts cliproxyexecutor.Options, routeModel string, execModels []string, pooled bool, aliasResult OAuthModelAliasResult) (*cliproxyexecutor.StreamResult, error) {
	return m.executeStreamWithModelPoolSignalExecutionModel(ctx, executor, auth, provider, req, opts, routeModel, "", execModels, pooled, aliasResult, executionResetSignal{}, nil)
}

func (m *Manager) executeStreamWithModelPoolExecutionModel(ctx context.Context, executor ProviderExecutor, auth *Auth, provider string, req cliproxyexecutor.Request, opts cliproxyexecutor.Options, routeModel, executionModel string, execModels []string, pooled bool, aliasResult OAuthModelAliasResult) (*cliproxyexecutor.StreamResult, error) {
	return m.executeStreamWithModelPoolSignalExecutionModel(ctx, executor, auth, provider, req, opts, routeModel, executionModel, execModels, pooled, aliasResult, executionResetSignal{}, nil)
}
...
func (m *Manager) executeStreamWithModelPoolSignal(ctx context.Context, executor ProviderExecutor, auth *Auth, provider string, req cliproxyexecutor.Request, opts cliproxyexecutor.Options, routeModel string, execModels []string, pooled bool, aliasResult OAuthModelAliasResult, signal executionResetSignal, onComplete func(success bool)) (*cliproxyexecutor.StreamResult, error) {
	...
	return m.executeStreamWithModelPoolSignalExecutionModel(ctx, executor, auth, provider, req, opts, routeModel, executionModel, execModels, pooled, aliasResult, signal, onComplete)
}

func (m *Manager) executeStreamWithModelPoolSignalExecutionModel(ctx context.Context, executor ProviderExecutor, auth *Auth, provider string, req cliproxyexecutor.Request, opts cliproxyexecutor.Options, routeModel, executionModel string, execModels []string, pooled bool, aliasResult OAuthModelAliasResult, signal executionResetSignal, onComplete func(success bool)) (*cliproxyexecutor.StreamResult, error) {
	if executor == nil {
	...
	for idx, execModel := range execModels {
		...
		if streamResult == nil {
			bootstrapErr = &Error{Code: "empty_stream", Message: "upstream returned no stream result", Retryable: true}
	...
	for {
		...
		for round := 0; round < maxRounds; round++ {
			...
			for {
				...
				if isRequestScopedResultError(resultErrorFromError(errExec)) || isRequestInvalidError(errExec) {
	...
	for {
		...
		for round := 0; round < maxRounds; round++ {
			...
			for {
				...
				if isRequestScopedResultError(resultErrorFromError(errStream)) || isRequestInvalidError(errStream) {
					return nil, errStream
				}
				lastErr = errStream
			}
...
func (m *Manager) executeMixedOnce(ctx context.Context, providers []string, req cliproxyexecutor.Request, opts cliproxyexecutor.Options, maxRetryCredentials int) (cliproxyexecutor.Response, error) {
	...
	for {
		...
		if errPrepare != nil {
			result := Result{AuthID: auth.ID, AuthGeneration: auth.RuntimeGeneration, Provider: provider, Model: routeModel, Success: false, Error: resultErrorFromError(errPrepare)}
		...
		for _, upstreamModel := range models {
			...
			result := Result{AuthID: auth.ID, AuthGeneration: auth.RuntimeGeneration, Provider: provider, Model: resultModel, Success: errExec == nil}
			if errExec != nil {
...
func (m *Manager) executeStreamMixedOnce(ctx context.Context, providers []string, req cliproxyexecutor.Request, opts cliproxyexecutor.Options, maxRetryCredentials int) (*cliproxyexecutor.StreamResult, error) {
	...
	for {
		...
		streamResult, errStream := m.executeStreamWithModelPoolExecutionModel(execCtx, executor, auth, provider, execReq, opts, routeModel, streamExecutionModel, models, pooled, aliasResult)
		if errStream != nil {
			if errCtx := execCtx.Err(); errCtx != nil {
				return nil, errCtx
			}
...
func (m *Manager) MarkResult(ctx context.Context, result Result) {
	...
	if auth, ok := m.auths[result.AuthID]; ok && auth != nil {
		...
		requestScopedFailure := !result.Success && isRequestScopedResultError(result.Error)
...
// preferredProviderForRequest evaluates the configured routing rules against the
// downstream request metadata (User-Agent, input character count) and returns the
// first matching provider that is present in the candidate provider list.
...
func (m *Manager) tryAntigravityCreditsExecute(ctx context.Context, req cliproxyexecutor.Request, opts cliproxyexecutor.Options) (cliproxyexecutor.Response, bool, error) {
	...
	for _, c := range candidates {
		...
		for _, upstreamModel := range models {
			...
			result := Result{AuthID: c.auth.ID, AuthGeneration: c.auth.RuntimeGeneration, Provider: c.provider, Model: resultModel, Success: errExec == nil}
			if errExec != nil {
				result.Error = resultErrorFromError(errExec)
				if ra := retryAfterFromError(errExec); ra != nil {
					result.RetryAfter = ra
