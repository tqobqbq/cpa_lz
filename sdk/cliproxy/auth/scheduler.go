...
type modelScheduler struct {
	...
	readyByPriority map[priorityTier]*readyBucket
...
func (s *authScheduler) pickMixed(ctx context.Context, providers []string, model string, opts cliproxyexecutor.Options, tried map[string]struct{}) (*Auth, string, error) {
	...
	for providerIndex, providerKey := range normalized {
		...
		if !hasCandidate || priorityReady.betterThan(bestPriority) {
...
func buildScheduledAuthMeta(auth *Auth) *scheduledAuthMeta {
	...
	return &scheduledAuthMeta{
		auth:              auth,
		providerKey:       providerKey,
		groupEnabled:      auth.GroupEnabled(),
		groupPriority:     auth.GroupPriority(),
...
func (m *scheduledAuthMeta) priorityTier() priorityTier {
	if m == nil {
...
func (p *providerScheduler) ensureModelLocked(modelKey string, now time.Time) *modelScheduler {
	...
	shard := &modelScheduler{
		...
		readyByPriority: make(map[priorityTier]*readyBucket),
	}
...
func (m *modelScheduler) upsertEntryLocked(meta *scheduledAuthMeta, now time.Time) {
	...
	previousParent := ""
	previousGroupEnabled := true
	previousWebsocketEnabled := false
	if entry.meta != nil {
		previousPriority = entry.meta.priorityTier()
		previousParent = entry.meta.virtualParent
		previousGroupEnabled = entry.meta.groupEnabled
	...
	if ok && previousState == entry.state && previousNextRetryAt.Equal(entry.nextRetryAt) && previousPriority == meta.priorityTier() && previousParent == meta.virtualParent && previousGroupEnabled == meta.groupEnabled && previousWebsocketEnabled == meta.websocketEnabled {
		return
	}
...
func (m *modelScheduler) promoteExpiredLocked(now time.Time) {
	...
	for _, entry := range m.blocked {
		...
		if entry.meta != nil && !entry.meta.groupEnabled {
			entry.state = scheduledStateDisabled
			entry.nextRetryAt = time.Time{}
			changed = true
			continue
...
// The caller must ensure expired entries are already promoted when needed.
func (m *modelScheduler) pickReadyAtPriorityLocked(preferWebsocket bool, priority priorityTier, strategy schedulerStrategy, predicate func(*scheduledAuth) bool) *Auth {
	if m == nil {
		return nil
	}
	bucket := m.readyByPriority[priority]
...
func (m *modelScheduler) readyCountAtPriorityLocked(preferWebsocket bool, priority priorityTier) int {
	if m == nil {
		return 0
	}
	bucket := m.readyByPriority[priority]
...
func (m *modelScheduler) rebuildIndexesLocked() {
	cursorStates := make(map[priorityTier]readyBucketCursorState, len(m.readyByPriority))
	...
	m.readyByPriority = make(map[priorityTier]*readyBucket)
	m.priorityOrder = m.priorityOrder[:0]
	m.blocked = m.blocked[:0]
	priorityBuckets := make(map[priorityTier][]*scheduledAuth)
	for _, entry := range m.entries {
		if entry == nil || entry.auth == nil {
			continue
		}
		...
		case scheduledStateReady:
			priority := entry.meta.priorityTier()
	...
	sort.Slice(m.priorityOrder, func(i, j int) bool {
		return m.priorityOrder[i].betterThan(m.priorityOrder[j])
	})
