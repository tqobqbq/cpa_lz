...
func (s *Service) applyCoreAuthAddOrUpdate(ctx context.Context, auth *coreauth.Auth) {
	...
	if existing, ok := s.coreManager.GetByID(auth.ID); ok {
		existingGeneration := existing.RuntimeGeneration
		existingDisabled := existing.Disabled || existing.Status == coreauth.StatusDisabled
		...
		if err == nil && updated != nil {
			updatedDisabled := updated.Disabled || updated.Status == coreauth.StatusDisabled
			rebindModels = existingGeneration == 0 ||
				updated.RuntimeGeneration != existingGeneration ||
				updatedDisabled != existingDisabled
			auth = updated
		}
	} else {
...
func (s *Service) applyCoreAuthRemoval(ctx context.Context, id string) {
	...
	if existing, ok := s.coreManager.GetByID(id); ok && existing != nil {
		...
		existing.Attributes["runtime_removed"] = "true"
		clearAuthRuntimeMetadata(existing)
		if _, err := s.coreManager.Update(coreauth.WithResetRuntimeState(ctx), existing); err != nil {
...
func clearAuthRuntimeMetadata(auth *coreauth.Auth) {
...
func (s *Service) applyRetryConfig(cfg *config.Config) {
	...
	s.coreManager.SetErrorControlConfig(cfg.ErrorControl)
}
...
func (s *Service) resolveConfigCodexKey(auth *coreauth.Auth) *config.CodexKey {
	...
	if idx, ok := configIndexFromAuth(auth, "codex", len(s.cfg.CodexKey)); ok {
		entry := &s.cfg.CodexKey[idx]
		cfgKey := strings.TrimSpace(entry.APIKey)
		cfgBase := strings.TrimSpace(entry.BaseURL)
		if attrKey != "" && strings.EqualFold(cfgKey, attrKey) {
			if cfgBase == "" || strings.EqualFold(cfgBase, attrBase) {
...
func configIndexFromAuth(auth *coreauth.Auth, provider string, length int) (int, bool) {
	if auth == nil || auth.Attributes == nil || length <= 0 {
		return 0, false
	}
	if expectedProvider := strings.TrimSpace(provider); expectedProvider != "" {
		actualProvider := strings.TrimSpace(auth.Attributes["config_provider"])
		if actualProvider != "" && !strings.EqualFold(actualProvider, expectedProvider) {
			return 0, false
		}
	}
	rawIndex := strings.TrimSpace(auth.Attributes["config_index"])
	if rawIndex == "" {
		return 0, false
	}
