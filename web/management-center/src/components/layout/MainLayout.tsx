...
import { AiProviderEditModal } from '@/components/providers/AiProviderEditModal';
import {
...
import { AI_PROVIDER_EDIT_MODAL_OPEN_EVENT } from '@/utils/aiProviderEditModal';
...
export function MainLayout() {
  ...
  const [aiProviderEditPath, setAiProviderEditPath] = useState<string | null>(null);
  ...
  useEffect(() => {
    const handleOpenAiProviderEdit = (event: Event) => {
      const path = (event as CustomEvent<string>).detail;
      if (path) {
        setAiProviderEditPath(path);
      }
    };

    window.addEventListener(AI_PROVIDER_EDIT_MODAL_OPEN_EVENT, handleOpenAiProviderEdit);
    return () => {
      window.removeEventListener(AI_PROVIDER_EDIT_MODAL_OPEN_EVENT, handleOpenAiProviderEdit);
    };
  }, []);
  ...
  return (
    <div className="app-shell">
      ...
      <AiProviderEditModal
        path={aiProviderEditPath}
        onClose={() => setAiProviderEditPath(null)}
        onNavigate={setAiProviderEditPath}
      />
    </div>
  );
}
