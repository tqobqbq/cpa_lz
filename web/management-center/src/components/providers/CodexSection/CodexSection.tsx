...
import {
  calculateStatusBarDataForIdentity,
...
export function CodexSection({
  ...
  const actionsDisabled = disableControls || loading || isSwitching || Boolean(testingKey);
  const toggleDisabled = disableControls || loading || isSwitching || Boolean(testingKey);

  const statusBarCache = useMemo(() => {
    const cache = new Map<string, ReturnType<typeof calculateStatusBarDataForIdentity>>();
    ...
    return cache;
  }, [configs, usage, usageDetailsByAuthIndex, usageDetailsBySource]);
      ...
      >
        <ProviderList<ProviderKeyConfig>
          ...
          renderExtraActions={(item, index) => (
            <>
              <Button
                ...
                loading={testingKey === `codex:${getProviderConfigKey(item, index)}`}
          ...
          renderContent={(item, index) => {
            ...
            const statusData = statusBarCache.get(getProviderConfigKey(item, index));

            return (
              <Fragment>
                <div className="item-title">{String(index)}</div>
                <div className={styles.fieldRow}>
                  <span className={styles.fieldLabel}>{t('common.api_key')}:</span>
                  <span className={styles.fieldValue}>{item.apiKey}</span>
                </div>
                ...
                <div className={styles.fieldRow}>
                  <span className={styles.fieldLabel}>{t('ai_providers.codex_use_v1_label')}:</span>
                  <span className={styles.fieldValue}>
                    {item.useV1 !== false ? t('common.yes') : t('common.no')}
                  </span>
                </div>
                {item.proxyUrl && (
                  <div className={styles.fieldRow}>
