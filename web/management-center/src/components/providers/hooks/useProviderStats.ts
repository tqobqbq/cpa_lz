...
export const useProviderStats = (options: UseProviderStatsOptions = {}) => {
  ...
  const usage = useUsageStatsStore((state) => (enabled ? state.usage : null));
  ...
  const loadKeyStats = useCallback(async () => {
    await loadUsageStats({ staleTimeMs: USAGE_STATS_STALE_TIME_MS, includeDetails: true });
  }, [loadUsageStats]);
  ...
  return { keyStats, usage, usageDetails, loadKeyStats, refreshKeyStats, isLoading };
};
