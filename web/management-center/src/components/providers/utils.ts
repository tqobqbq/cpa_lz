...
import {
  buildUsage20mIdentityCandidates,
  buildCandidateUsageSourceIds,
  calculateStatusBarData,
  calculateStatusBarDataFromUsage20m,
  normalizeAuthIndex,
  type KeyStatBucket,
  type KeyStats,
  type StatusBarData,
...
export const excludedModelsTextForEnabledState = (text: string, enabled: boolean) => {
  const models = parseExcludedModels(text);
  const nextModels = enabled
    ? withoutDisableAllModelsRule(models)
    : withDisableAllModelsRule(models);
  return excludedModelsToText(nextModels);
};
...
export const buildOpenAIResponsesEndpoint = (baseUrl: string, useV1 = true): string => {
  const trimmed = normalizeOpenAIBaseUrl(baseUrl);
  if (!trimmed) return '';
...
export const normalizeBackoffMode = (value: unknown): BackoffMode => {
...
export const normalizeRequestRetry = (value: unknown, fallback: number = 2): number => {
...
export const buildBackoffModeOptions = (
  t: TFunction
): Array<{ value: BackoffMode; label: string }> => [
  { value: 'default', label: t('ai_providers.backoff_mode_default') },
  { value: 'off', label: t('ai_providers.backoff_mode_off') },
  { value: 'custom', label: t('ai_providers.backoff_mode_custom') },
];

const escapeShellSingleQuotes = (value: string) => value.replace(/'/g, `'\\''`);

export const buildApiCallCommandPreview = (request: ApiCallRequest): string => {
  const lines = [
    `curl -X ${request.method.toUpperCase()} '${escapeShellSingleQuotes(request.url)}'`,
  ];
  if (request.proxyUrl) {
    lines.push(`  --proxy '${escapeShellSingleQuotes(request.proxyUrl)}'`);
  }
  const headers = Object.entries(request.header ?? {});
  headers.forEach(([key, value]) => {
    lines.push(`  -H '${escapeShellSingleQuotes(`${key}: ${value}`)}'`);
  });
  if (request.data) {
    lines.push(`  --data '${escapeShellSingleQuotes(request.data)}'`);
  }
  return lines.join(' \\\n');
};

export const formatApiCallResultPreview = (result: ApiCallResult): string => {
  const headers = Object.entries(result.header ?? {})
...
// Resolve usage stats by source/API key, matching the legacy behavior.
...
export const calculateStatusBarDataForIdentity = (
  identity: UsageIdentity,
  usageData: unknown,
  provider: string,
  usageDetailsBySource: UsageDetailsBySource,
  ...
  const bucketStatus = calculateStatusBarDataFromUsage20m(
    usageData,
    buildUsage20mIdentityCandidates(identity),
    { provider }
  );
  if (bucketStatus) {
    return bucketStatus;
...
export const calculateStatusBarDataForOpenAIProvider = (
  provider: OpenAIProviderConfig,
  usageData: unknown,
  usageDetailsBySource: UsageDetailsBySource,
  usageDetailsByAuthIndex: UsageDetailsByAuthIndex
): StatusBarData => {
  const identities: UsageIdentity[] = [];
  if (!provider.apiKeyEntries?.length) {
    identities.push({ authIndex: provider.authIndex, prefix: provider.prefix });
  } else {
    if (!normalizeAuthIndex(provider.authIndex) && provider.prefix) {
      identities.push({ prefix: provider.prefix });
    }
    provider.apiKeyEntries.forEach((entry) => {
      identities.push({ authIndex: entry.authIndex, apiKey: entry.apiKey });
    });
  }

  const bucketKeys = identities.flatMap((identity) => buildUsage20mIdentityCandidates(identity));
  const bucketStatus = calculateStatusBarDataFromUsage20m(usageData, bucketKeys, {
    provider: 'openai',
  });
  if (bucketStatus) {
    return bucketStatus;
...
// Aggregate all apiKeyEntries for OpenAI providers, matching the legacy behavior.
