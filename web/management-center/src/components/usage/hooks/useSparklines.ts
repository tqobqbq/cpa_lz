  ...
  const lastHourSeries = useMemo(() => {
    ...
    if (summaryEntries.length > 0) {
      summaryEntries.forEach((entry) => {
        if (!Number.isFinite(entry.bucketStartMs)) return;
        const minuteIndex = Math.min(
          windowMinutes - 1,
          Math.max(0, Math.floor((entry.bucketStartMs - windowStart) / 60000))
        );
        requestBuckets[minuteIndex] += Number(entry.stats.total_requests) || 0;
        tokenBuckets[minuteIndex] += usageBucketTokenBuckets(entry.stats).totalTokens;
      });
    } else {
