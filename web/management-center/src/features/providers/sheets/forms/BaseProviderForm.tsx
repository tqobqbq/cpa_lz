...
import type { ClaudeAuthMode, GeminiKeyConfig, OpenAIProviderConfig, ProviderKeyConfig } from '@/types';
...
function buildInitialForm(
  ...
  if (mode === 'create' || !resource) {
    return {
      ...
      useV1: brand === 'codex' ? true : undefined,
      authMode: brand === 'claude' ? 'auto' : undefined,
      cloak:
  ...
  return {
    ...
    useV1: brand === 'codex' ? (cfg as ProviderKeyConfig).useV1 !== false : undefined,
    authMode:
      brand === 'claude' ? ((cfg as ProviderKeyConfig).authMode ?? 'auto') : undefined,
    cloak:
      isClaudeLikeBrand(brand)
        ? {
...
export function BaseProviderForm({
  ...
  const supportsQuickImportPaste =
    mode === 'create' && (brand === 'codex' || brand === 'claude');

  const handleQuickImportTextChange = (text: string) => {
    setQuickImportText(text);
    if (!text.trim()) return;
    const extracted = extractQuickImportFields(text);
    if (extracted.baseUrl || extracted.apiKey) {
      setForm((prev) => ({
        ...prev,
        baseUrl: extracted.baseUrl || prev.baseUrl,
  ...
  return (
    <form id={formId} className={styles.form} onSubmit={handleSubmit} noValidate>
      ...
      <div className={styles.section}>
        {supportsQuickImportPaste ? (
          <div className={styles.field}>
            <label className={styles.label} htmlFor={`${fid}-quickImport`}>
              {t('ai_providers.quick_import_paste_label')}
            </label>
            <textarea
              id={`${fid}-quickImport`}
              ...
              disabled={mutating}
              onChange={(e) => handleQuickImportTextChange(e.target.value)}
            />
          </div>
        ) : null}
        ...
        {descriptor.supportsUseV1 ? (
          <label className={styles.checkboxRow}>
            <input
              ...
              disabled={mutating}
              onChange={(e) => updateField('useV1', e.target.checked)}
            />
            <span className={styles.checkboxText}>
              <span>
                {t('ai_providers.codex_use_v1_label')} · {t('ai_providers.codex_use_v1_hint')}
              </span>
            </span>
          </label>
        ) : null}

        {descriptor.supportsAuthMode ? (
          <div className={styles.field}>
            <label className={styles.label} htmlFor={`${fid}-authMode`}>
              {t('ai_providers.claude_auth_mode_label')}
              <span className={styles.labelHint}> · {t('ai_providers.claude_auth_mode_hint')}</span>
            </label>
            <Select
              id={`${fid}-authMode`}
              ...
              onChange={(value) => updateField('authMode', value as ClaudeAuthMode)}
              disabled={mutating}
