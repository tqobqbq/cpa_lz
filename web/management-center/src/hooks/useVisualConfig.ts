  ...
  const loadVisualValuesFromYaml = useCallback((yamlContent: string) => {
    try {
      ...
      const newValues: VisualConfigValues = {
        ...
        outputFilter: parseOutputFilter(asRecord(parsed['output-filter'])),
  ...
  const applyVisualChangesToYaml = useCallback(
    (currentYaml: string): string => {
      try {
        ...
        const defaultTestModelsDirty =
          dirtyFields.has('defaultTestModelCodex') || dirtyFields.has('defaultTestModelClaude');
        if (defaultTestModelsDirty) {
          ensureMapInDoc(doc, ['default-test-models']);
          if (dirtyFields.has('defaultTestModelCodex')) {
            setStringInDoc(doc, ['default-test-models', 'codex'], values.defaultTestModelCodex);
          }
          if (dirtyFields.has('defaultTestModelClaude')) {
