...
import type {
  ...
  PayloadRule,
  OutputFilterProviderRule,
  OutputFilterRuleValues,
  OutputFilterValues,
  ErrorControlPolicyField,
  ErrorControlPolicyValues,
  ErrorControlProviderPolicy,
...
function getPositiveNumberError(value: string): 'positive_number' | undefined {
  const trimmed = value.trim();
  if (!trimmed) return undefined;
  const parsed = Number(trimmed);
  return Number.isFinite(parsed) && parsed > 0 ? undefined : 'positive_number';
}
...
const ERROR_CONTROL_YAML_KEYS: Record<ErrorControlPolicyField, string> = {
  ...
  roundBackoffMax: 'round-backoff-max',
};
...
function parseErrorControlPolicy(raw: unknown): ErrorControlPolicyValues {
  const record = asRecord(raw) ?? {};
  const policy = createEmptyErrorControlPolicy();
  for (const field of ERROR_CONTROL_POLICY_FIELDS) {
    const value = record[ERROR_CONTROL_YAML_KEYS[field]];
    policy[field] = value === undefined || value === null ? '' : String(value);
  }
  return policy;
}

function parseErrorControlProviders(raw: unknown): ErrorControlProviderPolicy[] {
  const providers = asRecord(raw);
  if (!providers) return [];
  return Object.entries(providers).map(([provider, policy], index) => ({
    id: `error-control-provider-${index}`,
    provider,
    policy: parseErrorControlPolicy(policy),
  }));
}

function parseOutputFilterRule(raw: unknown): OutputFilterRuleValues {
  const record = asRecord(raw) ?? {};
  const rawKeywords = Array.isArray(record.keywords) ? record.keywords : [];
  const keywords = rawKeywords
    .map((keyword) => (keyword === undefined || keyword === null ? '' : String(keyword)))
    .filter((keyword) => keyword.trim() !== '');

  return {
    enabled: Boolean(record.enabled),
    maxLength: String(record['max-length'] ?? ''),
    keywordsText: keywords.join('\n'),
  };
}

function parseOutputFilterProviders(raw: unknown): OutputFilterProviderRule[] {
  const providers = asRecord(raw);
  if (!providers) return [];
  return Object.entries(providers).map(([provider, rule], index) => ({
    id: `output-filter-provider-${index}`,
    provider,
    ...parseOutputFilterRule(rule),
  }));
}

function parseOutputFilter(raw: unknown): OutputFilterValues {
  const record = asRecord(raw) ?? {};
  return {
    ...parseOutputFilterRule(raw),
    providers: parseOutputFilterProviders(record.providers),
  };
}

function splitOutputFilterKeywords(keywordsText: string): string[] {
  return keywordsText
    .split('\n')
...
function areOutputFilterRulesEqual(
  left: OutputFilterRuleValues,
  right: OutputFilterRuleValues
): boolean {
  return (
...
function areOutputFilterProvidersEqual(
  left: OutputFilterProviderRule[],
  right: OutputFilterProviderRule[]
): boolean {
  ...
  for (let i = 0; i < left.length; i += 1) {
    ...
    if (!areOutputFilterRulesEqual(a, b)) return false;
  }
  return true;
}

function areOutputFiltersEqual(left: OutputFilterValues, right: OutputFilterValues): boolean {
  return (
    areOutputFilterRulesEqual(left, right) &&
    areOutputFilterProvidersEqual(left.providers, right.providers)
  );
}

function isErrorControlPolicyEmpty(policy: ErrorControlPolicyValues): boolean {
  return ERROR_CONTROL_POLICY_FIELDS.every((field) => !policy[field].trim());
}

function areErrorControlPoliciesEqual(
  left: ErrorControlPolicyValues,
  right: ErrorControlPolicyValues
): boolean {
  return ERROR_CONTROL_POLICY_FIELDS.every((field) => left[field] === right[field]);
}

function areErrorControlProvidersEqual(
  left: ErrorControlProviderPolicy[],
  right: ErrorControlProviderPolicy[]
): boolean {
  if (left.length !== right.length) return false;
  for (let i = 0; i < left.length; i += 1) {
    const a = left[i];
    ...
    if (!a || !b) return false;
    if (a.id !== b.id || a.provider !== b.provider) return false;
    if (!areErrorControlPoliciesEqual(a.policy, b.policy)) return false;
  }
  return true;
}

function setErrorControlPolicyInDoc(
  doc: YamlDocument,
  path: YamlPath,
  ...
  setNumberFromStringInDoc(doc, [...path, 'round-backoff-max'], policy.roundBackoffMax);
  doc.deleteIn([...path, 'credential-backoff-stride']);
  doc.deleteIn([...path, 'credential-max-attempts']);
  deleteIfMapEmpty(doc, path);
}

function setErrorControlInDoc(doc: YamlDocument, values: VisualConfigValues): void {
  const hasDefault = !isErrorControlPolicyEmpty(values.errorControl.default);
  const providers = values.errorControl.providers.filter(
    (entry) => entry.provider.trim() && !isErrorControlPolicyEmpty(entry.policy)
  );
  if (!docHas(doc, ['error-control']) && !hasDefault && providers.length === 0) return;

  ensureMapInDoc(doc, ['error-control']);
  if (hasDefault || docHas(doc, ['error-control', 'default'])) {
    setErrorControlPolicyInDoc(doc, ['error-control', 'default'], values.errorControl.default);
  }

  if (providers.length > 0 || docHas(doc, ['error-control', 'providers'])) {
    ensureMapInDoc(doc, ['error-control', 'providers']);
    const existing = doc.getIn(['error-control', 'providers'], true);
    if (isMap(existing)) {
      for (const item of [...existing.items]) {
        const keyNode = item.key as { value?: unknown } | null | undefined;
        const key = String(keyNode?.value ?? '');
        if (!providers.some((entry) => entry.provider.trim() === key)) {
          doc.deleteIn(['error-control', 'providers', key]);
        }
      }
    }
    for (const entry of providers) {
      setErrorControlPolicyInDoc(
        doc,
        ['error-control', 'providers', entry.provider.trim()],
        entry.policy
      );
    }
    deleteIfMapEmpty(doc, ['error-control', 'providers']);
  }

  deleteIfMapEmpty(doc, ['error-control']);
}

function isOutputFilterRuleEmpty(rule: OutputFilterRuleValues): boolean {
  return (
    !rule.enabled &&
    !rule.maxLength.trim() &&
    splitOutputFilterKeywords(rule.keywordsText).length === 0
  );
}

function setOutputFilterRuleInDoc(
  doc: YamlDocument,
  path: YamlPath,
  rule: OutputFilterRuleValues
): void {
  const keywords = splitOutputFilterKeywords(rule.keywordsText);
  ensureMapInDoc(doc, path);
  setBooleanInDoc(doc, [...path, 'enabled'], rule.enabled);
  setIntFromStringInDoc(doc, [...path, 'max-length'], rule.maxLength);
  if (keywords.length > 0) {
    doc.setIn([...path, 'keywords'], keywords);
  } else if (docHas(doc, [...path, 'keywords'])) {
    doc.deleteIn([...path, 'keywords']);
  }
  deleteIfMapEmpty(doc, path);
}

function setOutputFilterProvidersInDoc(
  doc: YamlDocument,
  providers: OutputFilterProviderRule[]
): void {
  const providerRules = providers.filter(
    (entry) => entry.provider.trim() && !isOutputFilterRuleEmpty(entry)
  );
  if (providerRules.length === 0 && !docHas(doc, ['output-filter', 'providers'])) return;

  ensureMapInDoc(doc, ['output-filter', 'providers']);
  const existing = doc.getIn(['output-filter', 'providers'], true);
  if (isMap(existing)) {
    for (const item of [...existing.items]) {
      const keyNode = item.key as { value?: unknown } | null | undefined;
      const key = String(keyNode?.value ?? '');
      if (!providerRules.some((entry) => entry.provider.trim() === key)) {
        doc.deleteIn(['output-filter', 'providers', key]);
      }
    }
  }

  for (const entry of providerRules) {
    setOutputFilterRuleInDoc(doc, ['output-filter', 'providers', entry.provider.trim()], entry);
  }
  deleteIfMapEmpty(doc, ['output-filter', 'providers']);
}

function setOutputFilterInDoc(doc: YamlDocument, values: VisualConfigValues): void {
  const hadGlobalRule =
    docHas(doc, ['output-filter', 'enabled']) ||
    docHas(doc, ['output-filter', 'max-length']) ||
    docHas(doc, ['output-filter', 'keywords']);
  const hasGlobalRule = !isOutputFilterRuleEmpty(values.outputFilter);
  const hasProviderRules = values.outputFilter.providers.some(
    (entry) => entry.provider.trim() && !isOutputFilterRuleEmpty(entry)
  );

  if (!docHas(doc, ['output-filter']) && !hasGlobalRule && !hasProviderRules) return;

  ensureMapInDoc(doc, ['output-filter']);
  if (hadGlobalRule || hasGlobalRule) {
    setOutputFilterRuleInDoc(doc, ['output-filter'], values.outputFilter);
  } else {
    doc.deleteIn(['output-filter', 'enabled']);
    doc.deleteIn(['output-filter', 'max-length']);
    doc.deleteIn(['output-filter', 'keywords']);
  }
  setOutputFilterProvidersInDoc(doc, values.outputFilter.providers);
  deleteIfMapEmpty(doc, ['output-filter']);
}
...
export function getVisualConfigValidationErrors(
  ...
  for (const entry of values.outputFilter.providers) {
    errors[`outputFilter.providers.${entry.id}.maxLength` as VisualConfigFieldPath] =
  ...
  const setPolicyErrors = (prefix: string, policy: ErrorControlPolicyValues) => {
    ...
    errors[`${prefix}.roundBackoffBase` as VisualConfigFieldPath] = getPositiveNumberError(
      policy.roundBackoffBase
    );
    errors[`${prefix}.roundBackoffExponent` as VisualConfigFieldPath] = getPositiveNumberError(
      policy.roundBackoffExponent
    );
    errors[`${prefix}.roundBackoffMax` as VisualConfigFieldPath] = getPositiveNumberError(
      policy.roundBackoffMax
    );
  };
  setPolicyErrors('errorControl.default', values.errorControl.default);
  for (const entry of values.errorControl.providers) {
    setPolicyErrors(`errorControl.providers.${entry.id}`, entry.policy);
  }
  return errors;
}
...
export function parseVisualConfigValuesFromYaml(yamlContent: string): VisualConfigValues {
  ...
  const outputFilter = asRecord(parsed['output-filter']);
  const defaultTestModels = asRecord(parsed['default-test-models'] ?? parsed.defaultTestModels);
  const errorControl = asRecord(parsed['error-control']);
  ...
  return {
    ...
    codexRemoveEmptyInputName: Boolean(parsed['codex-remove-empty-input-name']),
    requestRetry: String(parsed['request-retry'] ?? ''),
    maxRetryInterval: String(parsed['max-retry-interval'] ?? ''),
    errorControl: {
      default: parseErrorControlPolicy(errorControl?.default),
      providers: parseErrorControlProviders(errorControl?.providers),
    },
    defaultTestModelCodex:
      typeof defaultTestModels?.codex === 'string' ? defaultTestModels.codex : '',
    defaultTestModelClaude:
      typeof defaultTestModels?.claude === 'string' ? defaultTestModels.claude : '',
    ...
    outputFilter: parseOutputFilter(outputFilter),
  };
}
...
export function applyVisualConfigValuesToYaml(
  ...
  try {
    ...
    setBooleanInDoc(doc, ['codex-remove-empty-input-name'], values.codexRemoveEmptyInputName);
    setIntFromStringInDoc(doc, ['request-retry'], values.requestRetry);
    doc.deleteIn(['max-retry-credentials']);
    setIntFromStringInDoc(doc, ['max-retry-interval'], values.maxRetryInterval);
    setErrorControlInDoc(doc, values);
    setOutputFilterInDoc(doc, values);
...
function getNextDirtyFields(
  ...
  if (Object.prototype.hasOwnProperty.call(patch, 'codexRemoveEmptyInputName')) {
    updateDirty(
      'codexRemoveEmptyInputName',
      nextValues.codexRemoveEmptyInputName === baselineValues.codexRemoveEmptyInputName
    );
  }
  ...
  if (Object.prototype.hasOwnProperty.call(patch, 'errorControl')) {
    updateDirty(
      'errorControl',
      areErrorControlPoliciesEqual(
        nextValues.errorControl.default,
        baselineValues.errorControl.default
      ) &&
        areErrorControlProvidersEqual(
          nextValues.errorControl.providers,
          baselineValues.errorControl.providers
        )
    );
  }
  if (Object.prototype.hasOwnProperty.call(patch, 'outputFilter')) {
    updateDirty(
      'outputFilter',
      areOutputFiltersEqual(nextValues.outputFilter, baselineValues.outputFilter)
    );
  }
  if (Object.prototype.hasOwnProperty.call(patch, 'defaultTestModelCodex')) {
    updateDirty(
      'defaultTestModelCodex',
      nextValues.defaultTestModelCodex === baselineValues.defaultTestModelCodex
    );
  }
  if (Object.prototype.hasOwnProperty.call(patch, 'defaultTestModelClaude')) {
    updateDirty(
      'defaultTestModelClaude',
      nextValues.defaultTestModelClaude === baselineValues.defaultTestModelClaude
    );
  }
