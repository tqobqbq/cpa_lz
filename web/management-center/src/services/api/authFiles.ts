...
const normalizeAuthFileEntry = (entry: AuthFileEntry): AuthFileEntry => {
  ...
  if (entry['auth_index'] !== undefined && normalized.authIndex === undefined) {
    normalized.authIndex = entry['auth_index'] as AuthFileEntry['authIndex'];
  }
  if (entry['runtime_only'] !== undefined && normalized.runtimeOnly === undefined) {
    normalized.runtimeOnly = entry['runtime_only'] as AuthFileEntry['runtimeOnly'];
  }
  if (entry['status_message'] !== undefined && normalized.statusMessage === undefined) {
    normalized.statusMessage = entry['status_message'] as AuthFileEntry['statusMessage'];
  }
  if (entry['proxy_url'] !== undefined && normalized.proxyUrl === undefined) {
    normalized.proxyUrl = String(entry['proxy_url'] ?? '').trim();
  }
  if (entry['backoff_mode'] !== undefined && normalized.backoffMode === undefined) {
    const raw = String(entry['backoff_mode'] ?? '').trim().toLowerCase();
    if (raw === 'default' || raw === 'off' || raw === 'custom') {
      normalized.backoffMode = raw;
    }
  }
  if (entry['request_retry'] !== undefined && normalized.requestRetry === undefined) {
    const parsed = Number(entry['request_retry']);
    if (Number.isFinite(parsed)) {
      normalized.requestRetry = parsed;
    }
  }
  return normalized;
};
...
export const authFilesApi = {
  ...
  patchFields: (payload: AuthFileFieldsPayload) =>
    apiClient.patch('/auth-files/fields', {
      ...
      backoff_mode: payload.backoffMode,
      request_retry: payload.requestRetry,
      note: payload.note,
    }),
