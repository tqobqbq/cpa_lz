...
export const USAGE_DETAILS_LIMIT = 100;
...
let inFlightUsageRequest:
  | { id: number; scopeKey: string; includeDetails: boolean; promise: Promise<void> }
  | null = null;
...
export const useUsageStatsStore = create<UsageStatsState>((set, get) => ({
  ...
  loadUsageStats: async (options = {}) => {
    ...
    if (
      ...
      inFlightUsageRequest.includeDetails === includeDetails
    ) {
    ...
    const detailsFresh =
      includeDetails &&
      !scopeChanged &&
      state.detailsLoadedAt !== null &&
      Date.now() - state.detailsLoadedAt < staleTimeMs;
    ...
    if (!force && ((includeDetails && detailsFresh) || (!includeDetails && summaryFresh))) {
      return;
    }
    ...
    const previousDetails = scopeChanged ? [] : state.usageDetails;
    ...
    const requestPromise = (async () => {
      try {
        const usageResponse = await usageApi.getUsage({
          includeDetails,
          detailsLimit: includeDetails ? USAGE_DETAILS_LIMIT : undefined,
        });
        ...
        const usageDetails = includeDetails ? collectUsageDetails(usage) : previousDetails;
        set({
          usage,
          ...
          detailsLoadedAt: includeDetails ? Date.now() : state.detailsLoadedAt,
          scopeKey
        });
      } catch (error: unknown) {
        if (requestId !== usageRequestToken) return;
    ...
    inFlightUsageRequest = { id: requestId, scopeKey, promise: requestPromise, includeDetails };
